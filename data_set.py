...
def print_data_set_header():
	divider = (f"+{'-'*8}+{'-'*19}+{'-'*8}+{'-'*10}+"
		f"{'-'*12}+{'-'*8}+{'-'*8}+{'-'*19}+")
	print(divider)
	print((f"|{'batch #':8s}|{'started at':19s}|{'batch %':8s}|{'# items':10s}|"
		f"{'# perms':12s}|{'time':8s}|{'% done':8s}|{'estimate':19s}|"))
	print(divider)
...
def print_batch_stats(idx, current_part, items, perms):
	...
	print((f"|{idx+1:8d}|{current_time}|{current_part:8.2f}|"
		f"{items:10d}|{perms:12d}|"), end="", flush=True)
...
class CombinedDataSet:
	...
	def __init__(self, data_sets, ctx):
		...
		self.total_dataset_perms = sum(
			[d.total_dataset_perms for d in data_sets])
		self.total_perms = self.total_dataset_perms
		self.idx = 0

		self.expressions_np = np.zeros(
			(self.rounded_n, NUM_TOKENS), dtype=np.int32)
		offset = 0
		for d in data_sets:
	...
	def setup_buffers(self, ctx):
		...
		self.data_set_sizes_np = np.array(
			[d.n for d in self.data_sets], dtype=np.int32)
		...
		self.data_set_start_idxs_g = cl.Buffer(ctx, 
			...
			hostbuf=self.data_set_start_idxs_np)
		...
		self.data_set_num_perms_g = cl.Buffer(ctx, 
			mf.READ_WRITE | mf.COPY_HOST_PTR, 
			hostbuf=self.data_set_num_perms_np)
	...
	def start_kernel(self, prg, queue):
		...
		cl.enqueue_copy(queue, 
			self.data_set_start_idxs_g, self.data_set_start_idxs_np)
		cl.enqueue_copy(queue, self.data_set_sizes_g, self.data_set_sizes_np)
		cl.enqueue_copy(queue, 
			self.data_set_num_perms_g, self.data_set_num_perms_np)

		self.event = prg.evaluate(queue, (self.rounded_n,), None, 
			self.expressions_g, self.result_g, 
			self.data_set_start_idxs_g, self.data_set_sizes_g, 
			self.data_set_num_perms_g, self.num_data_sets)
...
#		self.setup_buffers(ctx)
	...
	def setup_buffers(self, ctx):
		...
		self.data_set_start_idxs_np = np.array([
			0, self.rounded_n], dtype=np.int32)
...
#		print(f"Done with {100 * progress:6.2f}%")
	...
	def update_extra_stats(self, i, extra_stats):
		keys = ("division_fails", "subtraction_fails", "power_fails", 
			"permutation_fails", "permutation_successes")
		indices = (DIVISION_FAIL_INDEX, SUBTRACTION_FAIL_INDEX, 
			POWER_FAIL_INDEX, PERMUTATION_FAIL_INDEX, PERMUTATION_SUCCESS_INDEX)
